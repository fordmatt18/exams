...
# Import from custom modules using new names
import config
from logger_setup import setup_logging # Import the setup function
...
from model_builder import (
    ...
    # build_student_pattern_subproblem_model # Not directly used if using benders_core.solve_...
)
...
from benders_core import (
    solve_student_pattern_subproblem_task,
...
if __name__ == "__main__":
    ...
    setup_logging(config.LOG_FILE_NAME, config.LOG_LEVEL_CONSOLE, config.LOG_LEVEL_FILE, config_to_log, redirect_print=True)
    ...
    try:
        ...
        if not os.path.exists(config.STUDENT_ENROLLMENT_CSV_PATH):
            ...
            # This dummy data creation is simplified; ensure it matches expected format if used extensively.
            # For more complex dummy data, consider a dedicated function in data_utils.
            dummy_exams = [f'EXAM{k}' for k in range(1, 11)]
            num_dummy_students = 20
            dummy_data_list = []
            for i in range(1, num_dummy_students + 1):
                student_id = f's{i}'
                # Ensure patterns with NUM_EXAMS_IN_PATTERNS_FOR_OPTIMIZATION are created for testing
            ...
            # Ensure at least one pattern matches NUM_EXAMS_IN_PATTERNS_FOR_OPTIMIZATION if possible
            if config.NUM_EXAMS_IN_PATTERNS_FOR_OPTIMIZATION == 4: # Specific example from original
                 dummy_data_list[0]['exams'] = str(['EXAM1', 'EXAM2', 'EXAM3', 'EXAM4'])
                 if num_dummy_students > 1:
                    dummy_data_list[1]['exams'] = str(['EXAM1', 'EXAM2', 'EXAM3', 'EXAM4']) # Duplicate for count > 1
        ...
        if not all_time_slots: # Handles NUM_TIME_SLOTS = 0
        ...
        # --- Pre-generate Canonical Schedule Costs ---
        ...
        # This list contains unique patterns of size 'num_exams_for_optimization'.
        patterns_for_clash_constraints = target_student_pattern_list
                
        # --- Calculate Initial ('Before') Objective for ALL Target Patterns (Conditional) ---
        if config.CALCULATE_BEFORE_AFTER_OBJECTIVES:
            ...
            if canonical_costs_for_current_k is None and k_for_canonical_generation > 0: # Should be populated
            ...
            for p_tuple in target_student_pattern_list:
                per_instance_cost_initial = compute_expected_cost_for_student_pattern(
                    ...
                    print_details=config.PRINT_SUBPROBLEM_DETAILS_PER_ITERATION # For verbose eval
                )
        ...
        benders_master_model = build_benders_master_problem(
            all_exam_ids,
            all_time_slots,
            student_patterns_for_benders_optimization, # For theta variables
            target_student_pattern_counts, # Counts for the above patterns
            config.REWEIGHT_SUBPROBLEM_OBJECTIVE_BY_PATTERN_COUNT,
            config.RELAX_MASTER_PROBLEM_VARIABLES,
            config.ALLOW_INTRA_PATTERN_CONFLICTS, # The flag from config
            patterns_for_clash_constraints # The comprehensive list of patterns for the constraint
        )
        ...
        use_persistent_solver_local_flag = False # Default
        ...
        if config.USE_PARALLEL_SUBPROBLEM_SOLVING:
            process_pool_executor = concurrent.futures.ProcessPoolExecutor(max_workers=config.NUM_SUBPROBLEM_WORKERS)
        
        try:
            if process_pool_executor:
                process_pool_executor.__enter__() # Manage context for ProcessPoolExecutor
            ...
            while current_benders_iteration < config.MAX_BENDERS_ITERATIONS:
                ...
                logging.info(f"\n--- Benders Iteration {current_benders_iteration}/{config.MAX_BENDERS_ITERATIONS} "
                             f"(for {len(student_patterns_for_benders_optimization)} patterns) ---")
                ...
                if use_persistent_solver_local_flag and persistent_master_solver:
                    mp_results, mp_term_cond = solve_master_problem_persistent(
                        persistent_master_solver, benders_master_model,
                        suppress_solver_output=True, iteration_num=current_benders_iteration,
                        print_debug_flag=config.PRINT_MASTER_PROBLEM_DEBUG_PERSISTENT
                    )
                else:
                    mp_results, mp_term_cond = solve_pyomo_model(
                        benders_master_model, solver_name=config.MASTER_PROBLEM_SOLVER, suppress_solver_output=True
                    )
                ...
                current_master_schedule_solution = {
                    (e, t): (pyo.value(benders_master_model.exam_assignment_vars[e, t], exception=False)
                             if benders_master_model.exam_assignment_vars[e, t].value is not None else 0.0)
                    for e in all_exam_ids for t in all_time_slots
                }
                ...
                subproblem_task_arguments_list = []
                
                canonical_costs_for_current_k = precomputed_canonical_schedule_costs_by_k_exams.get(k_for_canonical_generation)
                if canonical_costs_for_current_k is None and k_for_canonical_generation > 0:
                    logging.error(f"Canonical costs for k={k_for_canonical_generation} not available for SP solving. Exiting loop.")
                    break 

                for p_tuple_sp in student_patterns_for_benders_optimization: 
                    student_count_for_sp = target_student_pattern_counts[p_tuple_sp]
                    sp_obj_weight = student_count_for_sp if config.REWEIGHT_SUBPROBLEM_OBJECTIVE_BY_PATTERN_COUNT else 1.0
                    
                    subproblem_task_arguments_list.append((
                        p_tuple_sp, all_time_slots, current_master_schedule_solution,
                        config.SUBPROBLEM_SOLVER, config.PRINT_SUBPROBLEM_DETAILS_PER_ITERATION,
                        sp_obj_weight, k_for_canonical_generation, 
                        canonical_costs_for_current_k
                    ))
                ...
                if config.USE_PARALLEL_SUBPROBLEM_SOLVING and process_pool_executor:
                    sp_results_iterable = process_pool_executor.map(solve_student_pattern_subproblem_task, subproblem_task_arguments_list)
                else:
                    for task_args in subproblem_task_arguments_list:
                        sp_results_iterable.append(solve_student_pattern_subproblem_task(task_args))
                
                for result_dict in sp_results_iterable:
                    ...
                    if config.REWEIGHT_SUBPROBLEM_OBJECTIVE_BY_PATTERN_COUNT:
                        ...
                        current_iteration_upper_bound_contribution += sp_cost * student_count_for_ub
                ...
                elif lower_bound > -float('inf'): # lb is a valid number
                ...
                if not any_subproblem_failed_this_iteration:
                    ...
                    for p_tuple_cut, sp_info in subproblem_results_this_iteration.items():
                        ...
                        if subproblem_obj_value == float('inf') or not subproblem_linking_duals: 
                            if config.PRINT_DUALS_LOOP: logging.debug(f"  Skipping cut for {p_tuple_cut}, obj={subproblem_obj_value}, no/empty duals.")
                            continue
                        
                        exams_in_pattern_for_cut = list(p_tuple_cut) 
                        benders_cut_variable_terms = []
                        benders_cut_constant_term = subproblem_obj_value
        ...
        # --- Calculate 'After' Objective (Conditional) ---
        if config.CALCULATE_BEFORE_AFTER_OBJECTIVES:
            ...
            if best_master_schedule_solution:
                ...
                if canonical_costs_for_current_k is None and k_for_canonical_generation > 0:
                    ...
                    for p_tuple_final in target_student_pattern_list:
                        per_instance_cost_final = compute_expected_cost_for_student_pattern(
                            p_tuple_final, all_time_slots, best_master_schedule_solution,
                            config.SUBPROBLEM_SOLVER,
                            k_for_canonical_generation, 
                            canonical_costs_for_current_k,
                            print_details=config.PRINT_SUBPROBLEM_DETAILS_PER_ITERATION # For verbose eval
                        ...
                        else: # Should be pre-populated
                            student_pattern_cost_tracking_dict[p_tuple_final] = {
                                'initial_total_cost': float('nan'), # Should have been set
                                'optimized_total_cost': total_cost_for_pattern_group_final,
                                'student_count': actual_student_count_final,
                                'is_in_optimization_subset': (p_tuple_final in student_patterns_for_benders_optimization)
                            }
                        ...
                        if per_instance_cost_final != float('inf'):
                            ...
                            break
                    logging.info(f"Overall 'after' objective for ALL {len(target_student_pattern_list)} target patterns: "
                                 f"{total_cost_all_target_patterns_after_optimization:.4f}")
            else:
                logging.warning("No best_master_schedule_solution found. Cannot calculate 'after' costs for all patterns.")
                for p_tuple_final in target_student_pattern_list: # Ensure 'optimized_total_cost' is marked
                     if p_tuple_final in student_pattern_cost_tracking_dict:
        ...
        # --- Summary Table (Conditional on before/after calculation for meaningful comparison) ---
        logging.info("\n--- Summary: Costs for ALL Target Patterns ---")
        results_df_list_all = []
        for p_tuple_res, data_res in student_pattern_cost_tracking_dict.items():
            results_df_list_all.append({
                'Pattern': str(p_tuple_res),
                'Student Count': data_res.get('student_count', 'N/A'),
                'In Opt Subset': data_res.get('is_in_optimization_subset', False),
                'Initial Total Cost': data_res.get('initial_total_cost', float('nan')), # Will be NaN if not calculated
                'Optimized Total Cost': data_res.get('optimized_total_cost', float('nan')) # Will be NaN if not calculated or no best schedule
            })
        if results_df_list_all:
            ...
            # For console, it might be too wide. Consider printing a subset or specific columns.
            ...
            if config.LOG_LEVEL_CONSOLE.upper() == "INFO": # Example: print a smaller version to console
                print("\n--- Summary Table (Key Columns - see log for full) ---")
                print(results_summary_df_all[['Pattern', 'Student Count', 'Initial Total Cost', 'Optimized Total Cost']].head(20).to_string(index=False, float_format="%.2f"))
